...
- Gezegen burç/derecelerini SADECE planets[Planet].sign ve planets[Planet].deg kullan — Venüs/Mars/Satürn/Jüpiter vb. burçlarını ASLA uydurma. Motor Venüs Akrep verdiyse Terazi yazma, Satürn Koç verdiyse Oğlak yazma.
...
def mock_interpret(engine_json: dict, lang="tr") -> str:
    ...
    def long_explain():
        ...
        harita = f"Haritan {asc_sig} yükseliyor ({asc_deg:.0f}°), Ay {moon.get('sign','')} burcunda." if asc_sig else ""
        if perf.get('type') != 'none' and perf.get('result') == 'YES':
            bag = "Aranızda sıcak ve akıcı bir bağ görünüyor."
        elif perf.get('type') != 'none':
            bag = "Aranızda bir temas var ama biraz pürüzlü."
        else:
