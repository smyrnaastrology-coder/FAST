...
# mobile: streamlit bagimliligi kaldirildi
...
class FBST_Engine:
    ...
    def __init__(self, p1, p2, event_date, event_time="12:00", city="Ankara", country="Türkiye", lat=39.9334, lon=32.8597, p1_isim="", p2_isim="", mod="es_sevgili", ebeveyn_rolu="anne", utc_offset=None):
        ...
        self._radar_top_para = []
        self._radar_top_huzur = []
        self._radar_top_tutku = []
        self._radar_top_kriz = []

    def set_radar_data(self, para=None, huzur=None, tutku=None, kriz=None):
        if para: self._radar_top_para = para
        if huzur: self._radar_top_huzur = huzur
        if tutku: self._radar_top_tutku = tutku
        if kriz: self._radar_top_kriz = kriz
        return self
        ...
        if self.mod == "ebeveyn_cocuk":
            ...
            try:
                ...
                if self._radar_top_para:
                    ...
                    for v in self._radar_top_para[:3]:
                        sehir_adi = v["sehir"]
                    ...
                    for v in self._radar_top_huzur[:3]:
                        sehir_adi = v["sehir"]
                        sehir_adi_sade = sehir_adi.split(",")[0].strip().split(" (")[0].strip()
                        if sehir_adi_sade not in BUYUK_SEHIRLER:
                            continue
                    ...
                    for v in self._radar_top_tutku[:2]:
                        sehir_adi = v["sehir"]
                        sehir_adi_sade = sehir_adi.split(",")[0].strip().split(" (")[0].strip()
                        if sehir_adi_sade not in BUYUK_SEHIRLER:
                            continue
                    ...
                    for v in self._radar_top_kriz[:2]:
                        sehir_adi = v["sehir"]
                        sehir_adi_sade = sehir_adi.split(",")[0].strip().split(" (")[0].strip()
                        if sehir_adi_sade not in BUYUK_SEHIRLER:
                            continue
            ...
            if self._radar_top_para:
                ...
                def add_top_5_to_pdf(baslik, liste_adi, skor_anahtari):
                    ...
                    filtrelenmis = []
                    _attr = '_' + liste_adi
                    for veri in (getattr(self, _attr, []) or []):
                ...
                for asteroit_adi in taranacak_asteroidler:
                    ...
                    for hedef_adi in hedef_gezegenler:
                        ...
                        if fark <= 5.0:
                            ...
                            _aktif_mod = self.mod or "es_sevgili"
