...
from core.data import (_FAST_RENKLER, fbst_yukselenler, fbst_retrolar,
    fbst_yukselenler_cocuk, fbst_retrolar_cocuk, fbst_yukselenler_ebeveyn, fbst_retrolar_ebeveyn,
    ...
    ARAP_EBEVEYN, ARAP_ILISKI, _load_ext_dict, _load_all_ext_dicts,
...
class FBST_Engine:
    ...
    def kadersel_cumle_kur(self, gezegen_ad, burc_ad, ev_no, rol=None):
        ...
        if rx_mi:
            if self.mod == "ebeveyn_cocuk":
                retro_sozluk = fbst_retrolar_cocuk if rol == "cocuk" else fbst_retrolar_ebeveyn
            else:
                retro_sozluk = fbst_retrolar
    ...
    def arap_noktasi_hesapla(self):
        ...
        if _aktif_mod == "ebeveyn_cocuk" and ARAP_EBEVEYN:
            ...
            _nokta_sozluk = ARAP_ILISKI.get("ARAP_NOKTALARI_ILISKI", {})
            _burc_sozluk = ARAP_ILISKI.get("ARAP_NOKTA_BURC_YORUMLARI", {})
            _ev_sozluk = ARAP_ILISKI.get("ARAP_NOKTA_EV_YORUMLARI", {})
    ...
    def arap_noktasi_sinastri_analizi(self):
        ...
        if _aktif_mod == "ebeveyn_cocuk" and ARAP_EBEVEYN:
            ...
            _nokta_sozluk = ARAP_ILISKI.get("ARAP_NOKTALARI_ILISKI", {})
            _sinastri_sozluk = ARAP_ILISKI.get("ARAP_NOKTA_SINASTRI_YORUMLARI", {})
            _gezegen_sinastri_sozluk = ARAP_ILISKI.get("ARAP_NOKTA_GEZEGEN_SINASTRI", {})
    ...
    def pdf_rapor_uret(self, dosya_adi="FBST_Kadersel_Kontrat.pdf"):
        ...
        from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, PageBreak, Spacer, Image, KeepTogether
        ...
        def baslik_karti_ekle(baslik_metni, alt_baslik=None, emoji="", hedef=None):
            ...
            """
            eklenenler.append(Spacer(1, 18))
            eklenenler.append(Paragraph(kart_html, styles['TurkishNormal']))
            eklenenler.append(Spacer(1, 10))

        def cerceveli_gorsel_ekle(dosya_yolu, genislik, yukseklik, baslik=None, hedef=None):
            """Görselleri ince altın çerçeve ile sararak ekler. hedef verilirse story yerine o listeye ekler."""
            eklenenler = hedef if hedef is not None else story
            if not os.path.exists(dosya_yolu):
                eklenenler.append(Paragraph(f"<i>Görsel bulunamadı: {dosya_yolu}</i>", styles['TurkishNormal']))
                return
            ...
            eklenenler.append(cerceveli)
            if baslik:
                eklenenler.append(Paragraph(f"<font color='#4A5568' size='8'><i>{baslik}</i></font>", styles['TurkishNormal']))
            eklenenler.append(Spacer(1, 8))
...
"""
        ...
        def gezegen_dokumu(j_gun, taraf_adi, tarih_str, harita_dosyasi, rol=None):
            ...
            if asc_burc != "Hesaplanamadi":
                if self.mod == "ebeveyn_cocuk":
                    if rol == "cocuk":
                        yorum = fbst_yukselenler_cocuk.get(asc_burc, "Bu vitrin henüz tanımlanmadı.")
                    else:
                ...
                # O Katmanda Mühür Varsa Altın Kutular İçinde Ekrana Bas
                    ...
                    for muhur in katman_muhurleri:
                        ...
                        for satir in muhur.split('\n'):
                            ...
                            if "Yıldız Bağlantısı" in temiz_satir or "Yıldız Hizalanması" in temiz_satir or "Güçlü Etki" in temiz_satir:
                                temiz_satir = f"<b><font color='#C9A96E'>{temiz_satir}</font></b>"
                            elif "Özel Yargı" in temiz_satir or "Yıldızın anlamı" in temiz_satir:
    ...
    # =========================================================================================
                    ...
                    for hedef_adi in hedef_gezegenler:
                        ...
                        if fark <= 5.0:
                            ...
                            if _aktif_mod == "ebeveyn_cocuk" and ASTEROID_SINASTRI_YORUMLARI_EBEVEYN:
                                yorum = ASTEROID_SINASTRI_YORUMLARI_EBEVEYN.get(yorum_key, _asteroid_sinastri_sozluk.get(yorum_key, varsayilan_yorum))
                            else:
        ...
        if self.mod == "ebeveyn_cocuk":
            # ═══ ANNE-ÇOCUK UYUM ÖZETİ ═══
            story.append(Spacer(1, 15))
            story.append(luks_cizgi_ekle(renk="#C9A96E", kalinlik=1.5))
            story.append(Spacer(1, 15))
            baslik_karti_ekle("ANNE-ÇOCUK UYUM ÖZETİ", alt_baslik="Annenin Güneş ve Ay konumuyla çocuğun iç dünyasının uyumu", emoji="🤱")
            story.append(Spacer(1, 8))
            try:
                cocuk_konum_uy = self.gezegen_konum_analizi()
                cocuk_gunes_uy = cocuk_konum_uy.get("Güneş")
                cocuk_ay_uy = cocuk_konum_uy.get("Ay")

                anne_jd_uy = self.get_natal_julian_day("p2")
                anne_poz_uy = {}
                for a_isim, a_gid in [("Güneş", swe.SUN), ("Ay", swe.MOON), ("Merkür", swe.MERCURY), ("Venüs", swe.VENUS), ("Mars", swe.MARS), ("Jüpiter", swe.JUPITER), ("Satürn", swe.SATURN)]:
                    try:
                        anne_poz_uy[a_isim] = swe.calc_ut(anne_jd_uy, a_gid)[0][0]
                    except Exception:
                        continue
                anne_gunes_uy = anne_poz_uy.get("Güneş")
                anne_ay_uy = anne_poz_uy.get("Ay")
                ...
                uyum_parcalari = []
                if anne_gunes_uy is not None and cocuk_ay_uy:
                    anne_gunes_burc = dereceyi_burca_cevir(anne_gunes_uy)
                    cocuk_ay_burc = cocuk_ay_uy.get('burc', '')
                ...
                if anne_ay_uy is not None and cocuk_gunes_uy:
                    anne_ay_burc = dereceyi_burca_cevir(anne_ay_uy)
                    cocuk_gunes_burc = cocuk_gunes_uy.get('burc', '')
                ...
                if anne_ay_uy is not None and cocuk_ay_uy:
                    anne_ay_burc2 = dereceyi_burca_cevir(anne_ay_uy)
                    cocuk_ay_burc2 = cocuk_ay_uy.get('burc', '')
                ...
                for uyum_parca in uyum_parcalari:
                    ...
                    story.append(uyum_kutu)
                    story.append(Spacer(1, 6))
            except Exception as e:
                story.append(Paragraph(f"<font color='red'>Anne-çocuk uyum özeti hatası: {str(e)}</font>", styles['Normal']))

            story.append(Spacer(1, 15))
            story.append(luks_cizgi_ekle(renk="#C9A96E", kalinlik=1.5))
            story.append(Spacer(1, 15))

            # ═══ ÖZEL BAĞ ANALİZİ (ANNE-ÇOCUK) ═══
            baslik_karti_ekle("ÖZEL BAĞ ANALİZİ", alt_baslik="Anne ve çocuk arasındaki kadersel mühürler ve ruh bağı", emoji="🔗")
            story.append(Spacer(1, 8))
            try:
                cocuk_konum_ob = self.gezegen_konum_analizi()
                anne_jd_ob = self.get_natal_julian_day("p2")
                gezegen_list_ob = [("Güneş", swe.SUN), ("Ay", swe.MOON), ("Merkür", swe.MERCURY), ("Venüs", swe.VENUS), ("Mars", swe.MARS), ("Jüpiter", swe.JUPITER), ("Satürn", swe.SATURN), ("Uranüs", swe.URANUS), ("Neptün", swe.NEPTUNE), ("Plüton", swe.PLUTO)]
                anne_poz_ob = {}
                for a_isim, a_gid in gezegen_list_ob:
                    try:
                        anne_poz_ob[a_isim] = swe.calc_ut(anne_jd_ob, a_gid)[0][0]
                    except Exception:
                        continue
                bag_acilari = []
                aci_tipleri_ob = [(0, "Kavuşum", 6), (60, "Sekstil", 4), (90, "Kare", 6), (120, "Üçgen", 6), (180, "Karşıt", 6)]
                for a_isim, a_pos in anne_poz_ob.items():
                    for c_isim in ["Güneş", "Ay", "Merkür", "Venüs", "Mars", "Jüpiter", "Satürn"]:
                        c_bil = cocuk_konum_ob.get(c_isim)
                        if not c_bil:
                            continue
                        c_pos = c_bil.get('ham_derece')
                        if c_pos is None:
                            continue
                        fark_ob = abs(a_pos - c_pos)
                        if fark_ob > 180:
                            fark_ob = 360 - fark_ob
                        for aci_d, aci_ad, orb_ob in aci_tipleri_ob:
                            fark2_ob = abs(fark_ob - aci_d)
                            if fark2_ob <= orb_ob:
                                bag_acilari.append({
                                    "anne": a_isim, "cocuk": c_isim, "aci": aci_ad,
                                    "orb": round(fark2_ob, 1),
                                })
                                break
                oncelik_ob = {"Kavuşum": 0, "Üçgen": 1, "Sekstil": 2, "Karşıt": 3, "Kare": 4}
                bag_acilari.sort(key=lambda x: (oncelik_ob.get(x["aci"], 5), x["orb"]))
                bag_parcalari = []
                if bag_acilari:
                ...
                for bag_parca in bag_parcalari:
                    ...
                    story.append(bag_kutu)
                    story.append(Spacer(1, 6))
            except Exception as e:
                story.append(Paragraph(f"<font color='red'>Özel bağ analizi hatası: {str(e)}</font>", styles['Normal']))
    ...
    def pdf_potansiyel_rapor_uret(self, dosya_adi=None):
        ...
        try:
            ...
            uo_g = self._get_utc_offset(d_g.year, d_g.month, d_g.day)
        ...
        try:
            ...
            zorluk_anne = [s for s in (potansiyel_anne_z or []) if s.get('aci_turu') in ('90', '180')]
            anne_rol_parcalari = []
            gorulen_rol = set()
            for satir in zorluk_anne[:6]:
                g1 = (satir['aci'] + "|").split("|")[0].split("-")[0].strip()
                if g1 in gorulen_rol:
                    continue
                gorulen_rol.add(g1)
        ...
        try:
            ...
            uo_ek = self._get_utc_offset(d_ek.year, d_ek.month, d_ek.day)
        ...
        try:
            ...
            if sinir_parcalari:
                ...
                sinir_kutu.setStyle(TableStyle([
                    ('BACKGROUND', (0, 0), (-1, -1), KART_ARKA_PLAN),
                    ('BOX', (0, 0), (-1, -1), 0.8, ALTIN_AMBER),
                    ('PADDING', (0, 0), (-1, -1), 10),
                ]))
        ...
        try:
            ...
            for g_isim in ["Güneş", "Ay", "Merkür", "Venüs", "Mars", "Jüpiter", "Satürn", "Uranüs", "Neptün", "Plüton"]:
                bil_k = konumlar_da.get(g_isim)
                if bil_k:
                    ev_no = bil_k.get('ev', 0)
                    if 1 <= ev_no <= 3:
                        kadran_sayac[1] += 1
                    elif 4 <= ev_no <= 6:
                        kadran_sayac[2] += 1
            ...
            kadran_kutu.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, -1), HexColor("#F7F3E9")),
                ('BOX', (0, 0), (-1, -1), 0.8, ALTIN_AMBER),
                ('PADDING', (0, 0), (-1, -1), 10),
            ]))
        ...
        try:
            ...
            tum_yildiz_dg = yildiz_dg.get("tum_yildizlar", []) if yildiz_dg else []
            yildiz_dg_parcalari = []
            for ys in tum_yildiz_dg[:10]:
                yildiz_ad = ys.get('yildiz', '')
                gezegen_dg = ys.get('gezegen', '')
                orb_dg = ys.get('orb', 0)
            ...
            d_ast = self.p1 if isinstance(self.p1, date) else datetime.strptime(str(self.p1), "%Y-%m-%d").date()
            uo_ast = self._get_utc_offset(d_ast.year, d_ast.month, d_ast.day)
            jd_ast = swe.julday(d_ast.year, d_ast.month, d_ast.day, self.saat_ondalik - uo_ast)
            ...
            for a_id, a_ad in asteroit_isimleri:
                try:
                    a_pos = swe.calc_ut(jd_ast, a_id)[0][0]
                except Exception:
                    continue
                for hedef_ad, hedef_pos in hedefler_ast.items():
                    if not hedef_pos:
                        continue
                    fark_ast = abs(a_pos - hedef_pos)
                    if fark_ast > 180:
        ...
        try:
            guclu_alanlar = []
            gorulen_g = set()
            for s in (potansiyel_sonuclari or []):
                if s['alan'] not in gorulen_g and len(guclu_alanlar) < 3:
                    gorulen_g.add(s['alan'])
                    guclu_alanlar.append(s['alan'])
            zorluk_3 = []
            gorulen_z3 = set()
            for s in (potansiyel_sonuclari or []):
                if s.get('aci_turu') in ('90', '180'):
                    anahtar = s['alan']
                    if anahtar not in gorulen_z3 and len(zorluk_3) < 3:
                        gorulen_z3.add(anahtar)
        ...
        try:
            ...
            try:
                if isinstance(self.p1, datetime):
                    dogum_t_z = self.p1.date()
                elif isinstance(self.p1, date):
                    dogum_t_z = self.p1
                else:
                    dogum_t_z = datetime.strptime(str(self.p1), "%Y-%m-%d").date()
            except Exception:
                dogum_t_z = None
            yas_z = None
            if dogum_t_z:
